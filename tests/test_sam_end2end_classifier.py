#!/usr/bin/env python3
"""
测试脚本：SAM（最大面积掩膜）叶片分割 + 第二阶段病灶分割模型
- 阶段1单独评估（SAM叶片分割）：PA, IoU-B, IoU-leaf, mIoU, 推理时间
...
def load_sam_model(checkpoint_path, model_type="vit_b", device="cuda"):
    ...
    # 低显存参数配置（与predict_sam_end2end.py一致）
...
def process_single_image(img_path, leaf_gt_path, lesion_full_path,
    ...
    - 病灶预测二值掩膜（全图） (bool)
    - 病灶真实二值掩膜（全图） (bool)
    - stage1 推理时间（秒，仅SAM生成+选择最大掩膜）
    - stage2 推理时间（秒，仅预处理+模型推理+resize回裁剪区域）
    """
    # 读取原始图像和真值
    ...
    if leaf_pred is None:
        leaf_pred = np.zeros((h, w), dtype=bool)
    ...
    pred2 = np.argmax(prob2, axis=0)                     # (target_size, target_size)
    ...
    # 映射回全图（不计入推理时间）
    lesion_pred_full = np.zeros((h, w), dtype=bool)
    lesion_pred_full[y1:y2+1, x1:x2+1] = (lesion_pred_cropped > 0)

    return (leaf_pred, leaf_true,
            lesion_pred_full, lesion_true_binary,
...
def main():
    ...
    parser.add_argument('--stage2_checkpoint', type=str, default=STAGE2_CHECKPOINT)
    ...
    for img_path, leaf_gt_path, lesion_full_path in tqdm(valid_pairs, desc="Evaluating"):
        (leaf_pred, leaf_true,
         lesion_pred, lesion_true,
         t1, t2) = process_single_image(
            img_path, leaf_gt_path, lesion_full_path,
            mask_generator, stage2_model, device, args.padding
        )
        ...
        true_healthy = leaf_true & (~lesion_true)
        true_lesion = lesion_true
        ...
        pred_healthy = leaf_pred & (~lesion_pred)
        pred_lesion = lesion_pred
    ...
    # 输出结果
    print("\n" + "="*70)
    print("Stage 1 (SAM Leaf Segmentation) - Binary Classification Results")
    print("="*70)
    ...
    print("="*70)
    print(f"Stage1 Inference Time (SAM):   {avg_time_stage1_ms:.2f} ms per image")
